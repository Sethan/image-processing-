...
"""
first the weight for the data is collected
then a subplot is set up
then looping trough the indices the image activation with the corrosponding weight is shown
"""
indices = [5, 8, 19, 22, 34]
weight_tensor = first_conv_layer.weight.data
print(weight_tensor.shape)
fig, axs=plt.subplots(2, 5, figsize=(10, 3))

t=0
for i in indices:
    axs[1][t].imshow(torch_image_to_numpy(activation[0][i]), cmap=plt.get_cmap('gray'))
    axs[0][t].imshow(torch_image_to_numpy(weight_tensor[i]))    	
    t=t+1
plt.show()
### END YOUR CODE HERE ### 
